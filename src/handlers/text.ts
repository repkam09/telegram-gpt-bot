...
import { sendMessageWrapper, updateChatContext, resetMemory, buildMessageArray, sendAdminMessage } from "../utils";
...
async function handleText(msg: TelegramBot.Message) {
    ...
    if (!await ChatMemory.hasName(userId)) {
        await ChatMemory.setName(userId, `${firstName} ${lastName} [${username}] [${userId}]`);
    }

    if (isGroupChat(msg)) {
        if (!await ChatMemory.hasName(chatId)) {
            await ChatMemory.setName(chatId, `${groupName} [${chatId}]`);
        }
    }
    ...
    if (Config.TELEGRAM_ID_WHITELIST) {
        ...
        if (!whitelist.includes(chatId)) {
            if (isPrivateChat(msg)) {
                ...
                await sendAdminMessage(`${firstName} ${lastName} [${username}] [${userId}] sent a message but is not whitelisted`);
                return;
            }
            ...
            if (isGroupChat(msg)) {
                ...
                await sendAdminMessage(`${firstName} ${lastName} [${username}] [${userId}] sent a message from group chat '${groupName} [${chatId}]' but the group is not whitelisted`);
                return;
            }
        }
    }
    ...
    const messages = await buildMessageArray(chatId, isGroupChat(msg), firstName, msg.text, groupName);

    // Ask OpenAI for the text completion and return the results to Telegram
    let response;
    let model = Config.OPENAI_API_LLM;
    try {
        if (await ChatMemory.hasLLM(chatId)) {
            model = await ChatMemory.getLLM(chatId);
        }
    ...
    if (response && response.data && response.data.choices) {
        const result = response.data.choices[0].message as ChatCompletionRequestMessage;
        ...
        await updateChatContext(chatId, result.role, result.content, result.role);
    }
}
...
// Add a command to reset the bots internal chat context memory
    ...
    if (message === "/debug") {
        if (Config.TELEGRAM_BOT_ADMIN === chatId) {
            const chatIds = Array.from(await ChatMemory.getNameKeys()).map(async (key) => await ChatMemory.getName(key));
            if (chatIds.length > 0) {
