...
export class HennosOpenAIProvider extends HennosBaseProvider {
    ...
    public async completion(req: HennosConsumer, system: HennosMessage[], complete: HennosMessage[]): Promise<HennosResponse> {
        ...
        return this.completionWithRecursiveToolCalls(req, messages);
    }

    private async completionWithRecursiveToolCalls(req: HennosConsumer, prompt: OpenAI.Chat.Completions.ChatCompletionMessageParam[]): Promise<HennosResponse> {
        const [tool_choice, tools, parallel_tool_calls] = getAvailableTools(req);
        ...
        try {
            ...
            if (response.choices[0].message.content) {
                if (response.choices[0].finish_reason === "length") {
                    ...
                    return this.completionWithRecursiveToolCalls(req, prompt);
                }
