...
export class SupabaseInstance {
    ...
    public static async init(): Promise<void> {
        ...
        try {
            ...
            if (error) {
                Logger.error(undefined, `Error initializing Supabase client: ${error.message}`);
                throw new Error(`Error initializing Supabase client: ${error.message}`);
            }

            Logger.info(undefined, "Supabase client initialized and connection verified");
        } catch (error) {
            Logger.error(undefined, `Error verifying Supabase client connection: ${(error as Error).message}`);
            throw new Error(`Error verifying Supabase client connection: ${(error as Error).message}`);
        }

        Logger.info(undefined, "SupabaseInstance initialized successfully");

    }

    public static async getUser(jwt: string): Promise<{ user: User | null; error: Error | null }> {
        if (!SupabaseInstance.client) {
            Logger.error(undefined, "Supabase client is not initialized");
        ...
        try {
            ...
            if (error || !data || !data.user) {
                Logger.warn(undefined, `Supabase getUser failed: ${error?.message || "No user data returned"}`);
                return { user: null, error: error || new Error("No user data returned") };
            }

            return { user: data.user, error: null };
        } catch (error) {
            Logger.error(undefined, `Error getting user from Supabase: ${(error as Error).message}`);
    ...
    public static async signInWithPassword(email: string, password: string): Promise<{ accessToken: string | null; error: Error | null }> {
        ...
        try {
            ...
            if (error || !data || !data.session) {
                Logger.warn(undefined, `Supabase sign-in failed: ${error?.message || "No session data returned"}`);
            ...
            Logger.error(undefined, `Error signing in with Supabase: ${(error as Error).message}`);
    ...
    public static async validate(jwt: string): Promise<{ valid: boolean }> {
        ...
        try {
            ...
            if (error || !data || !data.user) {
                Logger.warn(undefined, `Supabase validation failed: ${error?.message || "No user data returned"}`);
                return { valid: false };
            }

            return { valid: true };
        } catch (error) {
            Logger.error(undefined, `Error validating JWT with Supabase: ${(error as Error).message}`);
            return { valid: false };
        }
    }
