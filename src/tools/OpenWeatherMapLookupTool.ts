...
export class OpenWeatherMapLookupTool extends BaseTool {
    ...
    public static async callback(req: HennosConsumer, args: ToolCallFunctionArgs, metadata: ToolCallMetadata): Promise<ToolCallResponse> {
        ...
        Logger.info(req, `open_weathermap_lookup_tool_callback. ${JSON.stringify({ lat: args.lat, lon: args.lon, units: units, mode: mode })}`);
        try {
            if (mode === "current") {
            ...
            Logger.warn(req, `open_weathermap_lookup_tool_callback unknown mode. ${JSON.stringify({ lat: args.lat, lon: args.lon, units: units, mode: mode })}`);
            return [`open_weather_map_lookup error, unknown mode '${mode}' was specified`, metadata];
        } catch (err: unknown) {
            const error = err as Error;
            Logger.error(req, `open_weathermap_lookup_tool_callback error. ${JSON.stringify({ lat: args.lat, lon: args.lon, error: error.message })}`, error);
