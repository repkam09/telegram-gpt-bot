...
class HennosGoogleProvider extends HennosBaseProvider {
  ...
  public async completion(
    ...
    const chat = await getSizedChatContext(
      req,
      system,
      complete,
      Config.GOOGLE_LLM.CTX,
    );
    ...
    try {
      ...
      Logger.info(
        req,
        `Google Completion Success, Usage: ${calculateUsage(result.response.usageMetadata)}`,
      );
      ...
      while (calls && calls.length > 0 && maxIterations > 0) {
        const tool_calls = calls.map((tool_call) => {
          return [
            ...
            { name: tool_call.name },
          ] as [ToolCall, ToolCallMetadata];
        });

        const results = await processToolCalls(req, tool_calls);

        const shouldEmptyResponse = results.find(
          ([, , response]) => response?.__type === "empty",
        );
        ...
        Logger.info(
          req,
          `Google Completion Success, Usage: ${calculateUsage(result.response.usageMetadata)} (left=${maxIterations})`,
        );
      ...
      if (err instanceof GoogleGenerativeAIError) {
        Logger.info(
          req,
          `Google Completion Error: ${err.message}. Attempting OpenAI Fallback.`,
        );
        return HennosOpenAISingleton.instance().completion(
          req,
          system,
          complete,
        );
...
function convertHennosMessages(
  messages: HennosMessage[],
): ConvertHennosMessageResponse {
...
type HennosTextNonSystemMessage = {
  type: "text";
  role: "user" | "assistant";
  content: string;
};
function isHennosTextMessage(
  val: HennosMessage,
): val is HennosTextNonSystemMessage {
